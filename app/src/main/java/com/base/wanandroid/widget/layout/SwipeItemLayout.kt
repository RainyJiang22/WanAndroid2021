package com.base.wanandroid.widget.layout
...
class SwipeItemLayout @JvmOverloads constructor(context: Context?, attrs: AttributeSet? = null) : ViewGroup(context, attrs) {
    ...
    private fun ensureChildren(): Boolean {
        ...
        var childView: View? = getChildAt(0) as? ViewGroup ?: return true
        mMainView = childView as ViewGroup
        childView = getChildAt(1)
        if (childView !is ViewGroup) return true
        mSideView = childView
        return false
    }
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        var lp: MarginLayoutParams = mMainView!!.layoutParams as MarginLayoutParams
        ...
        if (heightMode == MeasureSpec.AT_MOST) heightSize =
            heightSize.coerceAtMost(mMainView!!.measuredHeight + verticalMargin + verticalPadding) else if (heightMode == MeasureSpec.UNSPECIFIED) heightSize =
        ...
        override fun run() {
            ...
            if (!mAbort) {
                ...
                if (!more) {
                    ...
                    //绝对不会出现这种意外的！！！可以注释掉
                    if (mScrollOffset != 0) {
                        mScrollOffset = if (abs(mScrollOffset) > mMaxScrollOffset / 2) -mMaxScrollOffset else 0
                        ViewCompat.postOnAnimation(this@SwipeItemLayout, this)
                    }
                }
            }
        ...
        override fun onInterceptTouchEvent(rv: RecyclerView, ev: MotionEvent): Boolean {
            ...
            when (action) {
                ...
                MotionEvent.ACTION_MOVE -> {
                    ...
                    val xDiff = abs(deltaX)
                    val yDiff = abs(deltaY)
    ...
    companion object {
        private val sInterpolator = (Interpolator { input ->
            val input1 = input - 1.0f
            return@Interpolator input1 * input1 * input1 * input1 * input1 + 1.0f
        })
